...
def mirrorlist_item(item):
    return MIRRORLIST_ITEM_TEMPLATE.format(title=mirror_title(item), **item)
...
def try_protocols(mirror):
    ...
    protocols = []
    print(
        'Accessing "{provider}" at "{domain}": ... '.format(domain=domain, **mirror),
        end="",
        flush=True,
    )
...
def main():
    ...
    geoparser = sub.add_parser("geo", help=f"update geo coordinates for {SOURCE_YAML}")
